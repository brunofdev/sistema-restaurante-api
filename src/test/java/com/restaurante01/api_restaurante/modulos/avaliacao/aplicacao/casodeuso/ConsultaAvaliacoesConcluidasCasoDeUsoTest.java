...
class ConsultaAvaliacoesConcluidasCasoDeUsoTest {
    ...
    @InjectMocks
    private ConsultaAvaliacoesConcluidasCasoDeUso casoDeUso;

    @Test
    @DisplayName("Dado que existem avaliações CONCLUIDA, Quando executar, Então retorna a lista mapeada")
    void deveRetornarAvaliacoesConcluidas() {
        Avaliacao avaliacao = AvaliacaoBuilder.umaAvaliacao().construir();
        AvaliacaoDTO dto = mock(AvaliacaoDTO.class);

        when(repositorio.buscarTodasPorStatus(StatusAvaliacao.CONCLUIDA)).thenReturn(List.of(avaliacao));
    ...
    @Test
    @DisplayName("Dado que não há avaliações CONCLUIDA, Quando executar, Então retorna lista vazia")
    void deveRetornarListaVaziaQuandoNaoHaConcluidas() {
        when(repositorio.buscarTodasPorStatus(StatusAvaliacao.CONCLUIDA)).thenReturn(Collections.emptyList());
    ...
    @Test
    @DisplayName("Dado qualquer cenário, Quando executar, Então busca somente avaliações com status CONCLUIDA")
    void deveBuscarApenasAvaliacoesConcluidas() {
        when(repositorio.buscarTodasPorStatus(StatusAvaliacao.CONCLUIDA)).thenReturn(Collections.emptyList());
        when(mapeador.mapearListaAvaliacoesDTO(any())).thenReturn(Collections.emptyList());

        casoDeUso.executar();

        verify(repositorio).buscarTodasPorStatus(StatusAvaliacao.CONCLUIDA);
        verify(repositorio, never()).buscarTodasPorStatus(StatusAvaliacao.PENDENTE);
    ...
    void deveDelegarMapeamentoAoMapeador() {
        ...
        when(repositorio.buscarTodasPorStatus(StatusAvaliacao.CONCLUIDA)).thenReturn(avaliacoes);
