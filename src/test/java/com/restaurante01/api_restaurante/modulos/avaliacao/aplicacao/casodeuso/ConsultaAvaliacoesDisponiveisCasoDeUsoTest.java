...
class ConsultaAvaliacoesDisponiveisCasoDeUsoTest {
    ...
    @InjectMocks
    private ConsultaAvaliacoesDisponiveisCasoDeUso casoDeUso;

    @Test
    @DisplayName("Dado que existem avaliações DISPONIVEL, Quando executar, Então retorna a lista mapeada")
    void deveRetornarAvaliacoesDisponiveis() {
        Avaliacao avaliacao = AvaliacaoBuilder.umaAvaliacao().construir();
        AvaliacaoDTO dto = mock(AvaliacaoDTO.class);

        when(repositorio.buscarTodasPorStatus(StatusAvaliacao.DISPONIVEL)).thenReturn(List.of(avaliacao));
    ...
    @Test
    @DisplayName("Dado que não há avaliações DISPONIVEL, Quando executar, Então retorna lista vazia")
    void deveRetornarListaVaziaQuandoNaoHaDisponiveis() {
        when(repositorio.buscarTodasPorStatus(StatusAvaliacao.DISPONIVEL)).thenReturn(Collections.emptyList());
    ...
    @Test
    @DisplayName("Dado qualquer cenário, Quando executar, Então busca somente avaliações com status DISPONIVEL")
    void deveBuscarApenasAvaliacoesDisponiveis() {
        ...
        verify(repositorio).buscarTodasPorStatus(StatusAvaliacao.DISPONIVEL);
    ...
    void deveDelegarMapeamentoAoMapeador() {
        ...
        when(repositorio.buscarTodasPorStatus(StatusAvaliacao.DISPONIVEL)).thenReturn(avaliacoes);
